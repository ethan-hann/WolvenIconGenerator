...
public class IconManager : IDisposable
{
        ...
        try
        {
            if (_instance != null)
                throw new InvalidOperationException(
                    "An instance of IconManager already exists. Use IconManager.Instance to access it.");
    ...
    public async Task InitializeAsync()
    {
        ...
        try
        {
            ...
            _wolvenKitCliDownloadUrl =
                $"https://github.com/WolvenKit/WolvenKit/releases/download/{WolvenKitVersion}/WolvenKit.Console-{WolvenKitVersion}.zip";
    ...
    public async Task<WolvenIcon?> GenerateIconImageAsync(string imagePath, string atlasName,
        IProgress<int>? progress = null, bool overwrite = true, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            AuLogger.GetCurrentLogger<IconManager>("GenerateIconImageAsync")
                .Info($"The icon import operation has started: {atlasName}");
            OnIconImportStarted(new StatusEventArgs($"The icon import operation has started: {atlasName}", false,
                _currentProgress));
            ...
            AuLogger.GetCurrentLogger<IconManager>("GenerateIconImageAsync")
                .Info($"The icon import operation has completed successfully: {icon?.AtlasName}");
            OnIconImportFinished(new StatusEventArgs(
                $"The icon import operation has completed successfully: {icon?.AtlasName}", false, _currentProgress));
        ...
        {
            AuLogger.GetCurrentLogger<IconManager>("GenerateIconImageAsync")
                .Info($"The icon import operation was cancelled: {atlasName}");
            OnIconImportStatus(new StatusEventArgs($"The icon import operation was cancelled: {atlasName}", false,
                _currentProgress));
        }
        catch (Exception e)
        {
    ...
    private async Task<WolvenIcon> CreateIconAsync(string imagePath, string atlasName, bool overwrite,
        CancellationToken token, IProgress<int>? progress)
    {
        try
        {
            ...
            WolvenIcon icon = new(projectImagePath)
            {
                CustomIcon =
                {
    ...
    /// <returns>A task that, when complete, returns a <see cref="WolvenIcon"/> or <c>null</c> if the operation is canceled or fails.</returns>
    /// <exception cref="InvalidOperationException">Occurs when the icon manager has not been initialized.</exception>
    public async Task<WolvenIcon?> ExtractIconImageAsync(string archivePath, IProgress<int>? progress = null,
        bool overwrite = true, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            AuLogger.GetCurrentLogger<IconManager>("ExtractIconImageAsync")
                .Info($"The icon export operation has started: {archivePath}");
            OnIconExportStarted(new StatusEventArgs($"The icon export operation has started: {archivePath}", false,
                _currentProgress));
            ...
            AuLogger.GetCurrentLogger<IconManager>("ExtractIconImageAsync")
                .Info($"The icon export operation has completed successfully: {icon?.AtlasName}");
            OnIconExportFinished(new StatusEventArgs(
                $"The icon export operation has completed successfully: {icon?.AtlasName}", false, _currentProgress));
        ...
        {
            AuLogger.GetCurrentLogger<IconManager>("ExtractIconImageAsync")
                .Info($"The icon export operation was cancelled: {archivePath}");
            OnIconExportStatus(new StatusEventArgs($"The icon export operation was cancelled: {archivePath}", false,
                _currentProgress));
        }
        catch (Exception e)
        {
    ...
    /// <param name="progress">An optional progress reporter for reporting progress.</param>
    /// <returns>A task containing the resulting <see cref="WolvenIcon"/>.</returns>
    private async Task<WolvenIcon> ExtractIconAsync(string archivePath, bool overwrite, CancellationToken token,
        IProgress<int>? progress)
    {
        try
        {
            ...
            await _wolvenKitCli!.UnpackArchiveAsync(archivePath, projectDirectories["projectBasePath"], token,
                progress);
            ...
            {
                OnIconExportStatus(new StatusEventArgs("No XBM files were found in the project folder.", true,
                    _currentProgress));
                CancelOperation();
            ...
            if (!File.Exists(newArchivePath))
            {
                OnIconExportStatus(new StatusEventArgs("The final .archive file could not be renamed.", true,
                    _currentProgress));
                CancelOperation();
                throw new FileNotFoundException("The final .archive file could not be renamed.", newArchivePath);
            }
            ...
            {
                OnIconExportStatus(new StatusEventArgs("No PNG files were found in the exported folder.", true,
                    _currentProgress));
                CancelOperation();
                throw new FileNotFoundException("No PNG files were found in the exported folder.",
                    projectDirectories["exportedPngs"]);
            }
            ...
            {
                OnIconExportStatus(new StatusEventArgs("The .inkatlas path and part could not be determined.", true,
                    _currentProgress));
                CancelOperation();
            ...
            // Create and return the WolvenIcon object
            WolvenIcon icon = new(pngFiles.First(), newArchivePath)
            {
                CustomIcon =
                {
    ...
    private async Task<(string inkAtlasPath, string inkAtlasPart)> GetInkAtlasPathAndPartAsync(string projectBasePath,
        string xbmFile, CancellationToken token, IProgress<int>? progress)
    {
        try
        {
    ...
    private void OnIconImportStarted(StatusEventArgs e)
    {
        IconImportStarted?.Invoke(this, e);
    }

    private void OnIconImportStatus(StatusEventArgs e)
    {
        CliStatus?.Invoke(this, e);
    }

    private void OnIconImportFinished(StatusEventArgs e)
    {
        IconImportFinished?.Invoke(this, e);
    }

    private void OnIconExportStarted(StatusEventArgs e)
    {
        IconExportStarted?.Invoke(this, e);
    }

    private void OnIconExportStatus(StatusEventArgs e)
    {
        CliStatus?.Invoke(this, e);
    }

    private void OnIconExportFinished(StatusEventArgs e)
    {
        IconExportFinished?.Invoke(this, e);
    }
